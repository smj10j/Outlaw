...
#else
    return (__bridge LHFixture*)fixtureA->GetUserData();
#endif
    return nil;
}
...
#else
    return (__bridge LHFixture*)fixtureB->GetUserData();
#endif
    return nil;
}

-(NSString*)fixtureNameA{
    LHFixture* fixA = [self contactFixtureA];
    if(fixA)
        return fixA.fixtureName;
    
    return @"";
}
-(NSString*)fixtureNameB{
    LHFixture* fixB = [self contactFixtureB];
    if(fixB)
        return fixB.fixtureName;
